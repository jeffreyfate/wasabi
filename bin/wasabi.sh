...
bootstrap() {
  if [ "${WASABI_OS}" == "${WASABI_OSX}" ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 58118E89F3A912897C070ADBF76221572C52609D >/dev/null 2>&1
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get update -qq
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -qq docker-ce docker-ce-cli containerd.io
