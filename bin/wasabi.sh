...
bootstrap() {
  if [ "${WASABI_OS}" == "${WASABI_OSX}" ]; then
    ...
    #Install git-flow
    DEBIAN_FRONTEND=noninteractive sudo apt-get install -y git-flow
    ...
    #Install compass
    DEBIAN_FRONTEND=noninteractive sudo apt-get install -y ruby
    DEBIAN_FRONTEND=noninteractive sudo apt-get install -y ruby-compass

    #Install docker
    DEBIAN_FRONTEND=noninteractive sudo apt-get install -y apt-transport-https ca-certificates
    ...
    sudo apt-get update
    DEBIAN_FRONTEND=noninteractive sudo apt-get install -y linux-image-extra-$(uname -r) linux-image-extra-virtual
