...
#   fixme: try to reuse the start_container() method instead of 'docker run...' directly; currently a problem with quotes in ${wenv} being passed into container.
  docker run --net=${docker_network} --name ${project}-main -p 80:8080 -p 8090:8090 -p 8180:8180 \
    -e "${wenv}" -d ${project}-main || \
    usage "docker run --net=${docker_network} --name ${project}-main -p 80:8080 -p 8090:8090 -p 8180:8180 -e \"${wenv}\" -d ${project}-main" 1
